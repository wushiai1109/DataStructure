 ...
 * ………………
 * @Author Bruce
 * @Date 2020/7/23 7:45 下午
 */
public class JZ33丑数 {

    public static void main(String[] args) {
        System.out.println(new JZ33丑数().GetUglyNumber_Solution(7));
    }
...
//        }
        ...
        while (linkedList.size() < index) {
            //选出三个队列头最小的数
            newNum = Math.min(linkedList.get(p2) * 2, Math.min(linkedList.get(p3) * 3, linkedList.get(p5) * 5));
            //这三个if有可能进入一个或者多个，进入多个是三个队列头最小的数有多个的情况
            if (linkedList.get(p2) * 2 == newNum) {
                p2++;
            }
            if (linkedList.get(p3) * 3 == newNum) {
                p3++;
            }
            if (linkedList.get(p5) * 5 == newNum) {
                p5++;
            }
            linkedList.addLast(newNum);
        }
